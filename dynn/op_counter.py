...
def measure_layer(layer, x, num_classes):
    ...
    delta_ops = 0
    delta_params = 0
    multi_add = 2 # 1 is MAD, 2 is FLOPS
    ...
    # elif type_name in ['GlobalFilter']:
    #     delta_ops = 
    ...
    ### ops_nonlinearity
        ...
        B = x.size()[0]
        C = x.size()[1]
        delta_ops = B*C*10*(a*b)*math.log2(a*b) + B*C*a*(b/2 + 1)*6
        delta_params = get_layer_param(layer)
    ### ops_pooling
...
def measure_model_and_assign_cost_per_exit(model, H, W, num_classes = 10):
    ...
    model.forward = model.forward_for_train
    # model.forward = model.forward_boost
    def should_measure(x):
        return is_leaf(x) or isinstance(x, LearnableUncGate)
    ...
    print(f"Model was successfully measured and exit costs were assigned {count_ops_at_exits}")
